...
label act_1_hospital_before_start:
    ...
    narrator "Поднявшись на ноги спустя пару десятков минут, ты замечаешь, что за окном уже давно рассвело."
    narrator "Конечно, брат Маттиас говорил, что мне "
...
label act_1_investigation_tavern:
    ...
    narrator "Таверна полна людьми. Застрявшие в городе возницы и торговцы, пьяные подмастерья и грубые стражники — все горячо увлечены обсуждением произошедшего."
...
label act_1_investigation_well_alley:
    ...
    narrator "Незнакомец молча указывает рукой вниз, словно призывая тебе заглянуть внутрь. Ты подходишь ближе."
...
label act_1_investigation_after_well:
    ...
    narrator "Альберт, будто ждал этого все это время, взахлеб рассказывает все подробности вашей встречи."
    ...
    menu:
        "Рассказать о видении":
            ...
            narrator "Ты рассказываешь Виллему о фигуре в капюшоне, на которую от тебе сегодня указал."
            willem "Ты про Лотара?"
            narrator "Хозяин кивает ту сторону, где утром был незнакомец."
            narrator "Там за столом сидит, а скорее уже лежит, глуповатого вида мертвецки пьяный стражник."
    ...
    menu:
        "Настоять на вылазке сегодня":
            ...
            willem "Она вышла вся в мать — и красотой, и своим ангельским голосом."
            willem "Жаль, что Ида не успела увидеть, как расцвела наша Эйлин."
...
label act_1_investigation_tavern_song:
    ...
    narrator "Пару часов ты дремлешь в зале, облокотившись на спику стула и укрывшись принесенным хозяином пледом"
    ...
    call cinematic_narrator("С первыми звуками лютни самые громкие голоса начинают затихать. Ты видишь, как несколько человек прерывают разговор и поворачивают головы в сторону Эйлин.", 7.0, 1.5)
    call cinematic_narrator("Когда ее нежный голос разносится по залу, наступает полная тишина. Все глаза устремляются к поющей девушке.", 7.0, 1.5)
    call cinematic_narrator("{i}О расскажи мне перед сном {i}\nО дальних островах", 7.5, 1.0, centered=True)
    call cinematic_narrator("{i}Там, за туманами, наш дом\nСокрыт в плавучих льдах.", 7.5, 1.0, centered=True)
    call cinematic_narrator("{i}По горным пикам ходит гордо{i}\nГрозный бог Борей.", 7.5, 1.0, centered=True)
    call cinematic_narrator("{i}И плачет над курганами\nТонкая свирель", 7.5, 1.0, centered=True)
    $ renpy.pause(2.0, hard=True)
    call cinematic_narrator("Пение Эйлин на краткий миг заставляет тебя забыть о тревогах. Ты сам не замечаешь, как закрываешь глаза и уносишся далеко отсюда...", 7., 1.5)
    call cinematic_narrator("Перед глазами всплывает отчий дом на берегу моря и разбивающиеся о камни волны...", 10.0, 1.5)
    call cinematic_narrator("Кажется, это все происходило слишком давно и с каким-то другим человеком...", 10.0, 1.5)
    $ renpy.pause(4.0, hard=True)
    call cinematic_narrator("Когда песня начинает затихать, ты с трудом возврашаешься в реальность. В глазах сидящих рядом с тобой зрителей стоят слезы восхищения.", 6.0, 1.5)
    ...
    narrator "Потом зал взрывается комплиментами. Девушка улыбается, кланяется и исчезает так же тихо, как появилась."
    narrator "Ты видишь, как поспешно Виллем отворачивается от зрителей, явно пытаясь сдержать слезы."
    narrator "Альберт рядом с ним сидит с открытым ртом, смотря на дверь, за которой скрылась Эйлин."
    narrator "Когда он пытается встать на ноги, то ты понимаешь, что паренек мертвецки пьян и едва держится на ногах."
...
label act_1_investigation_well_descent:
    ...
    narrator "Когда ты подходишь к колодцу, то воспоминания о произошедшем заставляют тебя на мгновение взрогнуть."
...
label act_1_investigation_inside_well:
    ...
    narrator "Ты упираешься руками в холодный камень и пытаешься надавить на плиту, но та не поддается ни на сантиметр."
    ...
    narrator "Кажется, безумное предроложение, что спящие могли уйти ночью через подземные ходы, может оказаться не таким и безумным..."
    narrator "Через пару минут, изучив остальную пещеру, ты понимамешь, что сейчас здесь делать больше нечего. Пора возвращаться."
...
label act_1_investigation_tavern_meeting:
    ...
    hide rykard guard unarmed with dissolve
    $ dream_char("willem default", [center])
...
label act_1_rykard_gang_counter:
    ...
    narrator "Их одежда слишком бедна для охраны богатого квартала, а руки слишком уверенно лежат у поясов, где под складками плащей угадываются ножи."

    if hero.prof == PROF.MONK:
        $ dream_char("gang leader default", [center])

        narrator "Главарь делает шаг вперед, но тут же замечает твое облачение."
        narrator "Его лицо два заметно меняется: насмешка все еще красуется на губах, но под ней проступает осторожность."
        ...
        hide gang leader default with dissolve
    ...
    if hero.prof == PROF.DOCTOR:
        ...
        if agatha_arrested_flag:
            ...
            narrator "От него пахнет дешевым элем, мокрой кожей и резким потом."
            ...
            narrator "Я думаю, пара сотен золотых монет будет в самый раз."
...
label act_1_rykard_gang_counter_guard_save:
    ...
    narrator "Ты успеваешь отшатнуться, но второй бандит уже оказывается рядом, и в свете фонаря на мгновение вспыхивает лезвие."
    narrator "Переулок сжимается вокруг тебя: мокрый камень под ногами, чужое дыхание за спиной, темные окна домов, за которыми никто не хочет ничего слышать."
    narrator "Еще секунда — и все заканчивается."
    narrator "Но из глубины улицы раздается сухой металлический щелчок."
    narrator "Бандиты замирают."
    narrator "У выхода из переулка стоит человек в доспехе, с арбалетом в руках. На его плаще виден знак дома Триллианов."
    narrator "Он не повышает голоса. Ему и не нужно."
...
label act_1_rykard_before_mansion:
    ...
    narrator "Кажется, что он совсем не вписывается в привычную архитектуру города, будто прибыл Морвейн из другой эпохи."
    ...
    if hero_saved_by_guard:
        ...
        narrator "Страж Рикарда, заметив твой восхищенный взгляд, короткой подталкивает тебя в сторону входа."
