...
label act_1_investigation_tavern_song:
    ...
    call cinematic_narrator("{i}В далекой стране {i}\nО дальних островах", 7.5, 1.0, centered=True)
    ...
    call cinematic_narrator("Ты сам не замечаешь, как перестаешь думать о тревогах и закрываешь глаза, ", 10.0, 1.5)
    $ renpy.pause(4.0, hard=True)
    call cinematic_narrator("Ты сам не замечаешь, как перестаешь думать о тревогах и закрываешь глаза, ", 10.0, 1.5)

    call cinematic_narrator("Постепенно зал смолкает. Кружки застывают в руках, разговоры обрываются, даже Виллем перестает протирать стойку.", 5.5, 2.0)
    call cinematic_narrator("У кого-то блестят глаза. Лотар приподнимается над столом и слушает так, словно боится пропустить хоть одну ноту.", 5.5, 2.0)
    call cinematic_narrator("Последняя нота гаснет, и несколько мгновений никто не решается нарушить тишину.", 4.5, 2.0)
    call cinematic_narrator("Потом зал взрывается комплиментами. Девушка улыбается, кланяется и исчезает так же тихо, как появилась.", 4.5, 2.0)

    hide screen soft_vignette
    with Dissolve(5.0)

    narrator "Ты видишь, как поспешно Виллем отворачивается от зрителей, явно пытаясь сдержать слезы."
    narrator "Альберт рядом с ним до сих пор сидит с открытым ртом, смотря на дверь, за которой скрылась Эйлин."
...
label act_1_investigation_inside_well:
    ...
    narrator "По её контуру едва заметно мерцают странные, почти потухшие символы в форме фаз луны."
    ...
    narrator "Кажется, безумная гипотеза, что спящие могли уйти ночью через подземные лазы, может оказаться не такой безумной..."
